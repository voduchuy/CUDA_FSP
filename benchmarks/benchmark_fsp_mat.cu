...
int main() {
    ...
    cuFSP::FSPMat A3
            (n_reactions, n_species, n_bounds, stoich, &t_func, &toggle_propensity_factor,
             cuFSP::KRONECKER);
    cudaDeviceSynchronize();
    CUDACHKERR();
    ...
    CUDACHKERR();
    thrust::fill(w.begin(), w.end(), 0.0);
    CUDACHKERR();

    thrust::fill(v.begin(), v.end(), 1.0);
    CUDACHKERR();
    ...
    for (int count = 0; count < 100; ++count){
        ...
        A2.action(1.0, (double *) thrust::raw_pointer_cast(&v[0]), (double *) thrust::raw_pointer_cast(&w[0]));
        cudaDeviceSynchronize();
        CUDACHKERR();

        A3.action(1.0, (double *) thrust::raw_pointer_cast(&v[0]), (double *) thrust::raw_pointer_cast(&w[0]));
        cudaDeviceSynchronize();
        CUDACHKERR();
    }
